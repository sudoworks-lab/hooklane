...
HooklaneはHTTP受付をdownstream配送から分離し、worker停止やretryable downstream failureの間もaccepted eventをqueueへ保持する必要がある。Local Composeとsingle-node kindで再現でき、queue、status、retry、pending recoveryを一つのdependencyで検証できる構成が必要だった。
...
- Kafkaなど別broker: local demonstrationのdependencyと運用複雑性が増え、必要contractに対して過剰なため採用しない。
