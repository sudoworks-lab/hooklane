...
- 2026-08-20: Cloudflare-native delivery backendはF001〜F029を変更しないpost-P0 local-only vertical spikeとして、`cloudflare/`配下へ既存Redis backendと分離して追加する。D1のtransactional event/outbox永続化を202 acceptance boundaryとし、Queue sendとの非原子性はrepair可能なoutboxと同一`event_id`の再送でloss-safeに扱う。Cloudflare Queuesへはpayloadではなく`event_id`だけを送り、Durable ObjectsとR2はcontract testが必要性を示さない限り採用しない。cloud deployment、credential、production domain、Tunnel、DNS、Accessはscope外とし、判断根拠と未保証事項は[ADR 0007](adr/0007-cloudflare-local-delivery-spike.md)を正本とする。
- 2026-08-20: Cloudflare portfolio completionは上記local-only spikeのsemantic gapだけを閉じるpost-P0 extensionとする。D1の2 MB single-row制約はpayloadを同一acceptance transaction内の複数rowへ分割して外部APIへ新規上限を追加せず回避し、R2との追加atomicity gapを導入しない。outbox repairはD1 compare-and-set leaseで並行ownerをclaimし、lease expiry後の同一`event_id`再送を許容してat-least-onceを維持する。Redisとのcontract／運用比較とproduction未保証事項をrepository evidenceとして追加するが、production telemetry、DLQ operator UI、cloud resource、Docker pin、commit、pushはscope外とする。
- 2026-08-21: Cloudflare CI productizationはpull request／mainの専用`cloudflare` jobを追加し、既存`quality`と並列preflight、`e2e-kind`を両gate成功後に実行する。jobは`.nvmrc`のNode 22.22.2、root Python 3.12.3、SHA-256固定uv 0.12.3、分離したCloudflare Python 3.13 environmentからminimal environmentを構築し、`make cloudflare-check`をrequired gateとして直接呼ぶ。source-only clean-roomとlocal CI contractまでは検証し、remote GitHub Actions成功、Cloudflare credential、remote resource、deployment、production trafficは主張しない。
- 2026-08-22: CI trust boundaryはrepository-local verifierだけに置かない。local contractはexact action revision、canonical concurrency、checked-out `GITHUB_SHA` identity、Cloudflare Make recipe、approved bootstrap source、CODEOWNERS coverageをaccidental weakening向けに検証する。external boundaryはbase branchのCODEOWNERSとpersonal public repositoryで利用可能なactive `main` ruleset、required Code Owner review、required status checksとし、GitHub Settingsで人間が有効化・確認するまでenforcement済みとは主張しない。single ownerのcontrol-plane更新はrepository administratorの`For pull requests only` bypassでPR記録を残す。
